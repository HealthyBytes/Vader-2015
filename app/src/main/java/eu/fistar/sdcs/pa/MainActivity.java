...
public class MainActivity extends FragmentActivity implements IPADialogListener {
    ...
    class LooperThread extends IProtocolAdapterListener.Stub implements Runnable {
        ...
        public void pushData(final List<Observation> observations, DeviceDescription deviceDescription) throws RemoteException {
            updateLog("Start writing observations.");
            List<String> someshit = new ArrayList<>();
            for (int i = 0; i < observations.size(); i++) {
                someshit = observations.get(i).getValues();
                updateLog(observations.get(i).getPropertyName() + observations.get(i).getMeasurementUnit());
                for(int j = 0; j < someshit.size(); j++){
                        updateLog(someshit.get(j));
                }
            }
            updateLog("Number of observations: " + observations.size());
            updateLog("Number of someshit: " + someshit.size());
            ...
            m_chartView.post(new Runnable()
            {
                ...
                public void run()
                {
                    ...
                    List<String> observation_values = observations.get(0).getValues();
                    Observation observation = observations.get(0);

                    for (String value : observation_values) {
